...
namespace Layout
{
    public partial class OriginalGraph : Form
    {
        ...
        public OriginalGraph(int[,] matrix, int[] groups)
        {
            InitializeComponent();
            ...
            for (int i = 0; i < matrix.GetLength(0); i++)
            {
                ...
                {
                    if (matrix[i, j] != 0 && !graphLink.Exists(u => (u.eX == j && u.eY == i)))
                    {
                        if (groups[0] > i)
                        {
            ...
            listPoint = new Point[matrix.GetLength(0)];
        ...
        private void OriginalGraph_Click(object sender, EventArgs e)
        {
            Refresh();

            using (Graphics graph = graphPictureBox.CreateGraphics())
            {
                using (Pen myPen = new Pen(Color.AliceBlue, 3))
                {
                    ...
                    foreach (var item in listPoint)
                    {
                        ...
                        graph.DrawString($"e{i + 1}", myFont, myBrush, new Point(item.X + 7, item.Y + 10));
        ...
        private void OriginalGraph_Activated(object sender, EventArgs e)
        {
            OnClick(e);
        }
